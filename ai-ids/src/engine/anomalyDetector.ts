...
export function computeIsolationScore(
  ...
  const weights = [0.17, 0.1, 0.08, 0.08, 0.12, 0.14, 0.08, 0.08, 0.05, 0.04, -0.06];
...
export function detectTLSAnomaly(log: NetworkLog, baseline: Baseline): AnomalySignal {
  ...
  return {
    ...
    description: detected
      ? `Observed unseen JA3-style fingerprint ${log.tlsFingerprint.slice(0, 12)}... on ${log.destIP}, with no matching baseline history.`
