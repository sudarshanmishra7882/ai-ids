...
function buildExecutiveSummary(
  ...
  if (alert === 'MEDIUM') {
    return `Summary: medium-confidence anomaly cluster (${threatCategory}) requires analyst validation. Confidence ${pct}%.`;
  }
  if (alert === 'HIGH') {
    return `Summary: high-confidence ${classification.replace(/_/g, ' ').toLowerCase()} pattern detected. Confidence ${pct}%.`;
  }
...
function buildExplanation(
  ...
  if (detectedSignals.length === 0) {
    return `Traffic from ${log.sourceIP} matches the learned ${getProfileLabel(inferBankingProfile(log))} baseline. No correlated anomaly chain is active; risk remains ${riskScore}/100 with ${(confidence * 100).toFixed(0)}% confidence.`;
  }
  ...
  return [
    `${threatCategory} observed from ${log.sourceIP}.`,
    statements.join(' '),
    correlationText,
...
export function correlateSignals(
  ...
  const explanation = buildExplanation(log, detectedSignals, threatCategory, correlationSignal, alert, confidence, Math.round(provisionalRisk));
  ...
  const executiveSummary = buildExecutiveSummary(alert, attackClassification, confidence, threatCategory);
