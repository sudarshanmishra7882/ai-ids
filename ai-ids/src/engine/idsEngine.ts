// ============================================================
// IDS ENGINE v4.0 — Multi-Agent Orchestrator / Streaming Pipeline
// Pipeline: Packet Agent → Flow Agent → Behavior Agent → Risk Engine → Correlation → Response
// ============================================================
...
export function processLog(state: SystemState, timestamp: number): {
  ...
  // 5. RUN MULTI-AGENT PIPELINE (v4.0)
  ...
  const coreAgentSignals = [packetSignal, flowSignal, behaviorSignal];

  // 6. TIME-DECAYED RISK ACCUMULATION (v4.0)
  const isolationScore = computeIsolationScore(log, state.baseline, windowStats);
  const newRiskState = accumulateRisk(state.riskState, coreAgentSignals, isolationScore, timestamp);
  ...
  // 8. Response Agent runs AFTER correlation with full context
  ...
  const agentSignals = [...coreAgentSignals, responseSignal];
