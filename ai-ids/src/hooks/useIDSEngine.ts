...
const TICK_INTERVAL_MS = 750; // sub-second streaming
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const apiResp = {
        ...
        detectionModes: result.detectionModes,
        signatureMatches: result.signatureMatches,
        zeroDayScore: result.zeroDayScore,
        clusterOutlierScore: result.clusterOutlierScore,
        nearestBehaviorCluster: result.nearestBehaviorCluster,
        autoIsolationRecommended: result.autoIsolationRecommended,
      };
