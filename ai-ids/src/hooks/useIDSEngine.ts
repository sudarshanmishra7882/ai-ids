...
import { SystemState, DetectionResult, WindowStats } from '../types/ids';
import {
  createInitialState,
  processLog,
  triggerAttack,
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const apiResp = {
        ...
        timestamp,
        meta: {
          isolationForestScore: result.isolationScore,
          correlationScore: result.correlationScore,
          falsePositiveReduced: result.falsePositiveReduced,
          windowSizeSeconds: 60,
          baselineSamples: newState.baseline.samplesCollected,
        },
      };
