...
export const LiveLogFeed: React.FC<LiveLogFeedProps> = ({ history }) => {
  ...
  return (
    <div className="bg-slate-900/60 rounded-xl border border-slate-700/50 flex flex-col">
      {/* Header */}
      ...
      {/* Table header */}
      <div className="grid grid-cols-12 gap-1 px-4 py-2 text-xs font-mono text-slate-600 uppercase tracking-wider border-b border-slate-800">
        <span className="col-span-2">Time</span>
        <span className="col-span-2">Source IP</span>
        <span className="col-span-2">Dest IP</span>
        <span className="col-span-1">Q/min</span>
        <span className="col-span-1">Pkt</span>
        <span className="col-span-1">Risk</span>
        <span className="col-span-2">Alert</span>
      ...
      >
        {reversed.length === 0 ? (
          ...
          reversed.map((entry, idx) => (
            <div
              key={entry.log.id}
              className={`grid grid-cols-12 gap-1 px-4 py-1.5 text-xs font-mono border-b border-slate-800/50 transition-colors ${ROW_BG[entry.alert]} ${idx === 0 ? 'animate-pulse-once' : ''}`}
            >
              <span className="col-span-2 text-slate-500">
                {format(new Date(entry.log.timestamp), 'HH:mm:ss')}
              </span>
              <span className="col-span-2 text-slate-400 truncate">{entry.log.sourceIP}</span>
              <span className={`col-span-2 truncate ${entry.log.isExternalIP ? 'text-orange-400' : 'text-slate-400'}`}>
                {entry.log.destIP}
              </span>
              <span className={`col-span-1 ${entry.log.queryFrequency > 100 ? 'text-red-400 font-bold' : 'text-slate-400'}`}>
                {entry.log.queryFrequency}
              </span>
              <span className={`col-span-1 ${entry.log.packetSize > 1200 ? 'text-orange-400' : 'text-slate-400'}`}>
